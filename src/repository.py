...
class SqlRepository(Generic[ModelType, CreateModelType, UpdateModelType], ABC):
    ...
    async def get_all(
        self, *conditions: Any, offset: int = 0, limit: int = 100
    ) -> Sequence[ModelType]:
    ...
    async def update(
        self, id: str | UUID, model_data: UpdateModelType, *conditions: Any
    ) -> Optional[ModelType]:
    ...
    def _query_builder(
        self, *conditions: Any, query: Optional[Select[Any]] = None
    ) -> Select[Any]:
        if query is None:
