...
def partition_file_exist_stmt(
    partition_id: uuid.UUID, file_id: uuid.UUID
) -> Select[Any]:
    return (
        select(PartitionFile)
        .where(PartitionFile.partition_id == partition_id)
        .where(PartitionFile.file_id == file_id)
    )


def get_tool_collection(collection_id: Union[uuid.UUID, str]):
    return f"{str(collection_id)}_tool"
