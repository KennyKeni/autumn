...
from llama_index.core import StorageContext, SummaryIndex, VectorStoreIndex, load_index_from_storage # pyright: ignore[reportUnknownVariableType]
...
class SummaryToolHandler(FileToolHelper):
    ...
    def create_tool(
        ...
        **kwargs: Unpack[CreateToolArgs],
        ...
        summary_index: BaseIndex[Any] = load_index_from_storage( # pyright: ignore[reportUnknownVariableType]
            storage_context=storage_context, 
            index_id=str(partition_file_tool.id)
        )
        summary_index = assert_isinstance(summary_index, SummaryIndex)
        summary_query_engine: BaseQueryEngine = summary_index.as_query_engine( # pyright: ignore[reportUnknownMemberType]
        ...
        summary_tool = QueryEngineTool.from_defaults(
            ...
            description=cls.create_tool_description(partition_file_tool.partition_file.file.name),
        )
        ...
        def vector_query(
            ...
            qdrant_filter = {
                "must": [
                    ...
                    create_partition_filter(str(partition_file_tool.partition_file.partition_id)),
                ]
            }
